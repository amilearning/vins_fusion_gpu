...
ros::Publisher pub_camera_geo_pose;
nav_msgs::Path path;
...
void registerPub(ros::NodeHandle &n)
{
    ...
    pub_camera_geo_pose = n.advertise<geometry_msgs::PoseStamped>("cam_pose", 1000);    
    pub_mavros_pose = n.advertise<geometry_msgs::PoseStamped>("/mavros/vision_pose/pose", 1000);   
...
void pubOdometry(const Estimator &estimator, const std_msgs::Header &header)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        //need to transform from body to px4 base_link
        Eigen::Matrix4d world_to_body_;           
        world_to_body_.block<3,3>(0,0) = estimator.Rs[WINDOW_SIZE];         
        world_to_body_.block<3,1>(0,3) = estimator.Ps[WINDOW_SIZE];
        Eigen::Matrix4d world_to_px4; 
        world_to_px4 = world_to_body_*body_to_px4;                
        Quaterniond tmp_quat_;
        tmp_quat_ = Quaterniond(world_to_px4.block<3,3>(0,0)).normalized();
        odometry.pose.pose.orientation.x = tmp_quat_.x();
        odometry.pose.pose.orientation.y = tmp_quat_.y();
        odometry.pose.pose.orientation.z = tmp_quat_.z();
        odometry.pose.pose.orientation.w = tmp_quat_.w();
        odometry.pose.pose.position.x = world_to_px4.block<1,1>(0,3).value();
        odometry.pose.pose.position.y = world_to_px4.block<1,1>(1,3).value();
        odometry.pose.pose.position.z = world_to_px4.block<1,1>(2,3).value();
        pose_stamped.pose = odometry.pose.pose;
        path.header = header;
        path.header.frame_id = "map";        
        pub_mavros_pose.publish(pose_stamped);
        //
        ...
        pub_camera_geo_pose.publish(pose_stamped);
